...
## Запуск на компьютере (Desktop Browser)

Игра теперь поддерживает запуск в обычном браузере компьютера (Chrome/Safari/Edge/Firefox):

1. Запустите статический сервер:
```bash
python3 -m http.server 8080
```
2. Откройте на компьютере:
`http://localhost:8080`
3. Управление мышкой работает во всех мини-играх.
